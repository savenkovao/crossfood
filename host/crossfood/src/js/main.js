(function($){
	...
	// cuisine switching 
		...
		function selectCuisineItem() {
			...
			$('#form-2_price').val(cuisineListener.price);
	...
	// reviewsSlider browsing/opening
		...
		function selectOtions (targetNode) {
			...
			if (cuisineListener.subscribe == 'fit') {
					...
					dataSub = 'veg';
			} else if (cuisineListener.subscribe == 'stand') {
		...
		$('#form_2 form').on('change', function (event) {
			...
			var form = document.forms[1];
			var selectSubscribe = form.elements.menu77.value;
			var selectCalories = form.elements.menu29.value;
			...
			if (selectSubscribe == 'Фитнес') {
					...
					selectSubscribe = 'veg';
			} else if (selectSubscribe == 'Стандартный') {
					selectSubscribe = 'stand';
			} else if (selectSubscribe == 'Пробный день') {
					selectSubscribe = 'trial';
			}

			if (selectSubscribe != 'trial') {
				cuisineListener.price = $('#'+selectSubscribe+'-'+cuisineListener.week+'-'+selectCalories+'-'+'mo'+' .cuisine__item-price-text'+'[data-cuisine-qount = 5]').html();
			} else if(selectSubscribe == 'trial'){
				cuisineListener.price = $('#'+selectSubscribe).html();
			}
