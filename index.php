<?php get_header() ?>
	<section class="how-it-works">
		<div class="wrapper  how-it-works_position">
			
			<div>
				<p class="descript">
					<u class="descript__highlited">CrossFood &mdash;</u> это команда профессионалов, которая возьмет на себя заботу о твоем рационе. Мы подбираем для тебя меню, считаем КБЖУ, готовим и привозим в удобное для тебя время. Больше не надо ходить в магазины, стоять у плиты и мыть посуду. 
				</p>
			</div>				

			<div id="how-it-works" class="title-3 how-it-works_title-3-position">
				<h3 class="title-3__h3 title-3__h3_dark hr-line">Как это работает</h3>	
			</div>	

			<div class="how-it-works_steps-pos">
				<div class="steps">
					<p class="steps__item steps__item-1">Выбор тарифа, идеально подходящего под твои цели и задачи</p>
					<p class="steps__item steps__item-2">Приготовление еды для тебя и порциям</p>
					<p class="steps__item steps__item-3">Бесплатная доставка в любую точку города каждое  утро</p>
					<p class="steps__item steps__item-4">Твоё правильно питание без усилий на протяжении всего дня</p>
					<img class="steps__img-desktop" src="<?php bloginfo("template_url"); ?>/src/img/dsg/steps.png" alt="Crossfood - как это работает">
					<img class="steps__img-mobile" src="<?php bloginfo("template_url"); ?>/src/img/dsg/steps-mobile.png" alt="Crossfood - как это работает">
				</div>
			</div>
			
		</div>
	</section>

	<section id="advantages" class="advantages advantages_background">
		<div class="wrapper advantages_position">
			<div class="title-3">
				<h3 class="title-3__h3 title-3__h3_light hr-line">Преимущества</h3>	
			</div>

			<div class="advantage-block advantages_advantage-block-pos">
				<div class="advantage-block__item"> 
					<p class="advantage-block__descript advantage-block__descript-1">
						Все приёмы сбалансированы по БЖУ и калорийности
					</p>
				</div>
				<div class="advantage-block__item"> 
					<p class="advantage-block__descript advantage-block__descript-2">
						Меню составлено профессиональными диетологами
					</p>
				</div>
				<div class="advantage-block__item"> 
					<p class="advantage-block__descript advantage-block__descript-3">
						Экономия времени на приготовление пищи и покупку продуктов
					</p>
				</div>
			</div>


		</div>
	</section>


	<section id="our-team" class="our-team">
		<div class="wrapper our-team_position">
			<div class="title-3">
				<h3 class="title-3__h3 title-3__h3_dark hr-line">Наша команда</h3>	
			</div>

			<div id="team-slider" class="team-block our-team_team-block-pos">

				<div data-action="sl" data-meta-node class="team-block__item sl"> 
					<img class="team-block__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/team/item-0.jpg" alt="Жак Фреско - хороший парень">					
					<div class="team-block__text">
						<h5 class="team-block__name team-block__name-1">
							Жак Фреско
						</h5>
						<p class="team-block__descript team-block__descript-1">
							хороший парень
						</p>
					</div>						
				</div>
				<div data-action="ml" data-meta-node class="team-block__item ml"> 
					<img class="team-block__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/team/item-1.jpg" alt="Мария Саввина - нутрициолог">					
					<div class="team-block__text">
						<h5 class="team-block__name team-block__name-1">
							Мария Саввина
						</h5>
						<p class="team-block__descript team-block__descript-1">
							нутрициолог
						</p>
					</div>						
				</div>
				<div data-action="l" data-meta-node class="team-block__item l"> 
					<img class="team-block__img team-b" src="<?php bloginfo("template_url"); ?>/src/img/cnt/team/item-2.jpg" alt="Иван Воробьёв - шеф">
					<div class="team-block__text">
						<h5 class="team-block__name team-block__name-2">
							Иван Воробьёв
						</h5>
						<p class="team-block__descript team-block__descript-2">
							шеф
						</p>
					</div>
				</div>
				<div data-action="mr" data-meta-node class="team-block__item mr"> 					
					<img class="team-block__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/team/item-3.jpg" alt="Ольга Янгичер - диетолог">
					<div class="team-block__text">
						<h5 class="team-block__name team-block__name-3">
							Ольга Янгичер
						</h5>
						<p class="team-block__descript team-block__descript-3">
							диетолог
						</p>
					</div>
				</div>
				<div data-action="sr" data-meta-node class="team-block__item sr"> 					
					<img class="team-block__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/team/item-4.jpg" alt="Петр Иванов - член команды">
					<div class="team-block__text">
						<h5 class="team-block__name team-block__name-3">
							Петр Иванов
						</h5>
						<p class="team-block__descript team-block__descript-3">
							член команды
						</p>
					</div>
				</div>
			</div>
		</div>
	</section>


	<section class="subscribe subscribe_background">
		<div class="wrapper subscribe_position">
			<div class="title-3">
				<h3 class="title-3__h3 title-3__h3_dark hr-line">Планы подписок</h3>	
			</div>

			<div class="tab-block subscribe_tab-block-pos">

				<div  id="subscribe" class="tab-block__navigation">
					<div data-action="data-subscribe" data-identifier="fit" data-meta-node data-highlight="subscribe__nav-button_active" class="tab-block__nav-button subscribe__nav-button subscribe__nav-button_active">Fitnes</div>
					<div data-action="data-subscribe" data-identifier="prem" data-meta-node data-highlight="subscribe__nav-button_active" class="tab-block__nav-button subscribe__nav-button">Premium</div>
				</div>

				<div id="subscribe-items" class="tab-block__tabs subscribe_tabs-position">
					
					<div data-subscribe="fit" class="tab-block__tab-item">
						
						<div data-identifier="sl" data-meta-node data-highlight="subscribe__item-active" class="subscribe__item subscribe__item-col-2 subscribe__item-active">
							<h4 class="subscribe__item-title">Slim</h4>
							<ul class="subscribe__item-text">
								<li class="subscribe__item-text-li"><strong>1200-1400 ккал</strong></li>	
								<li class="subscribe__item-text-li">3/5 блюд в день</li>
								<li class="subscribe__item-text-li">Доставка по дням</li>
								<li class="subscribe__item-text-li">300/400 грн на день</li>
								<li class="subscribe__item-text-li">4 000/5 500 грн на 14 дней</li>
							</ul>

							<div class="subscribe__button-cnt">
								<button class="button button_bordered subscribe__button-btn">Заказать</button>
							</div>

						</div>
						<div data-identifier="pow" data-meta-node data-highlight="subscribe__item-active" class="subscribe__item subscribe__item-col-2">
							<h4 class="subscribe__item-title">Power</h4>
							<ul class="subscribe__item-text">
								<li class="subscribe__item-text-li"><strong>От 1800 ккал</strong></li>	
								<li class="subscribe__item-text-li">3/5 блюд в день</li>	
								<li class="subscribe__item-text-li">Доставка по дням</li>
								<li class="subscribe__item-text-li">300/400 грн на день</li>
								<li class="subscribe__item-text-li">4 000/5 500 грн на 14 дней</li>
							</ul>

							<div class="subscribe__button-cnt">
								<button class="button button_bordered subscribe__button-btn">Заказать</button>
							</div>
						</div>

					</div>

					<div data-subscribe="prem" class="tab-block__tab-item disable" >
						
						<div data-identifier="reg" data-meta-node data-highlight="subscribe__item-active" class="subscribe__item subscribe__item-col-2 subscribe__item-active">
							<h4 class="subscribe__item-title">Regular</h4>
							<ul class="subscribe__item-text">
								<li class="subscribe__item-text-li">Меню - обычное</li>
								<li class="subscribe__item-text-li">3/5 блюд в день</li>
								<li class="subscribe__item-text-li">На 14 дней</li>
								<li class="subscribe__item-text-li">Количество ккал - индивидуально</li>								
								<li class="subscribe__item-text-li">Доставка по дням</li>
								<li class="subscribe__item-text-li">Стоимость - от 4 000 грн</li>
							</ul>

							<div class="subscribe__button-cnt">
								<button class="button button_bordered subscribe__button-btn">Заказать</button>
							</div>
			
						</div>

						<div data-identifier="veg" data-meta-node data-highlight="subscribe__item-active" class="subscribe__item subscribe__item-col-2">
							<h4 class="subscribe__item-title">Vegetarian</h4>
							<ul class="subscribe__item-text">
								<li class="subscribe__item-text-li">Меню - вегетарианское</li>
								<li class="subscribe__item-text-li">3/5 блюд в день</li>
								<li class="subscribe__item-text-li">На 14 дней</li>
								<li class="subscribe__item-text-li">Количество ккал - индивидуально</li>								
								<li class="subscribe__item-text-li">Доставка по дням</li>
								<li class="subscribe__item-text-li">Стоимость - от 3 500 грн</li>
							</ul>

							<div class="subscribe__button-cnt">
								<button class="button button_bordered subscribe__button-btn">Заказать</button>
							</div>
			
						</div>

					</div>

				</div>	

			</div>
		</div>
	</section>

	<section class="cuisine cuisine_background">
		<div class="wrapper cuisine_position">
			<div class="title-3">
				<h3 class="title-3__h3 title-3__h3_light hr-line">Наше меню</h3>	
			</div>

			<div class="tab-block cuisine-block_position">

				<div id="cuisine-week" class="tab-block__navigation cuisine-block__navigation_pos">
					<div data-identifier="mo" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button cuisine-block__button_active">

						<span class="cuisine-block__button-desktop">Понедельник</span>
						<span  class="cuisine-block__button-mobile">Пн</span>

					</div>
					<div data-identifier="tu" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Вторинк</span>
						<span  class="cuisine-block__button-mobile">Вт</span>

					</div>
					<div data-identifier="we" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Среда</span>
						<span  class="cuisine-block__button-mobile">Ср</span>

					</div>
					<div data-identifier="th" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Четверг</span>
						<span  class="cuisine-block__button-mobile">Чт</span>

					</div>
					<div data-identifier="fr" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Пятница</span>
						<span  class="cuisine-block__button-mobile">Пт</span>

					</div>
					<div data-identifier="sa" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Суббота</span>
						<span  class="cuisine-block__button-mobile">Сб</span>

					</div>
					<div data-identifier="su" data-meta-node data-highlight="cuisine-block__button_active" class="tab-block__nav-button cuisine-block__button">

						<span class="cuisine-block__button-desktop">Воскресенье</span>
						<span  class="cuisine-block__button-mobile">Вс</span>

					</div>
				</div>
					
				<div id="cuisines_3_5" class="tab-block__3-5-cuisines">
					<span data-action="data-cuisine-qount" data-identifier="3" data-meta-node data-highlight="tab-block__3-5-cuisines_active" class="tab-block__3-cuisines">3 блюда</span>
					<span> / </span>
					<span data-action="data-cuisine-qount" data-identifier="5" data-meta-node data-highlight="tab-block__3-5-cuisines_active" class="tab-block__5-cuisines tab-block__3-5-cuisines_active">5 блюд</span>
				</div>

				<div id="all-cuisines" class="tab-block__tabs cuisine__tabs-position">



				

						<div id="fit-sl-mo" class="tab-block__tab-item">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Оладьи из тыквы на цельнозерновой муке с ягодами Годжи  и суперпышный омлет «Облако»</li>
											<li>Рыба в панировке из льна на пару со свежим огурцом и  Диким Рисом</li>
											<li>Греча с грибами и куриной котлетой фаршированной адыгейским сыром</li>
											<li>Печеная свекла с салатом айсберг и запеченным с Киви филе домашней курицы</li>
											<li>Творожная запеканка с маком и миндальным орехом</li>
										</ol>


										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Рыба в панировке из льна на пару со свежим огурцом и  Диким Рисом</li>
											<li>Греча с грибами и куриной котлетой фаршированной адыгейским сыром</li>
											<li>Печеная свекла с салатом айсберг и запеченным с Киви филе домашней курицы</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-mo.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-tu" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Овсяноблин со сливочным соусом с индейкой грибами и корнем сельдерея</li>
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай  «Массала»  на натуральном сахарозаменителе</li>
											<li>Пшеничная каша с пряными травами и рулетики из судака с сыром и зеленью</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
											<li>Творожная Бабка с Курагой и мятой </li>
										</ol>


										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки - 110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай  «Массала»  на натуральном сахарозаменителе</li>
											<li>Пшеничная каша с пряными травами и рулетики из судака с сыром и зеленью</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-tu.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-we" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Рис на молоке с  ароматом Ванили печеными фруктами и жаренным арахисом</li>
											<li>Кус-кус по Мароккански с паровыми  фрикадельками из индейки</li>
											<li>Греча с творогом и куриное филе гриль с рюмкой крепкого бульона с зеленью</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты ... + Чай Fitness c имбирем и лимоном </li>
											<li>Нежирный творог с овощной пряной сальсой</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Кус-кус по Мароккански с паровыми  фрикадельками из индейки</li>
											<li>Греча с творогом и куриное филе гриль с рюмкой крепкого бульона с зеленью</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты ... + Чай Fitness c имбирем и лимоном </li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-we.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-th" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Сырники без муки на Псиллиуме , подаются с   ванильно-клубничным соусом на йогурте</li>
											<li>Целые отборные фрукты</li>
											<li>Салат Цезарь на легком Фитнесс соусе с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс  с морепродуктами и томатами Черри</li>
											<li>Творожная запеканка с овощами и корениями</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Целые отборные фрукты</li>
											<li>Салат Цезарь на легком Фитнесс соусе с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс с морепродуктами и томатами Черри</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-th.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-fr" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Тосты в банане и яйце с ягодами и сметанным соусом с медом и семенами Чиа</li>
											<li>Протеиновая конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Нешлифованный Рис с телятиной в томатном соусе</li>
											<li>Кубинский салат с ананасом, филе, яблоком и йогуртом</li>
											<li>Творожный маффин с черникой и семенами льна</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Протеиновая конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Нешлифованный Рис с телятиной в томатном соусе</li>
											<li>Кубинский салат с ананасом, филе, яблоком и йогуртом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-fr.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-sa" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Омлет с овощами и индейкой Су-Вид и сыром</li>
											<li>Апельсиновый фреш</li>
											<li>Отварное куриное филе с гречей, овощами и морковкой Су-Вид</li>
											<li>Салат из шпината, огурца на паровом филе морского окуня </li>
											<li>Творожная запеканка с печеными овощами и травами </li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Апельсиновый фреш</li>
											<li>Отварное куриное филе с гречей, овощами и морковкой Су-Вид</li>
											<li>Салат из шпината, огурца на паровом филе морского окуня </li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-sl-su" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Блины с грушей и миндальным орехом + нежирная сметана с семенами льна</li>
											<li>Отварная телятина, дайкон и лаваш «СrossFood»</li>
											<li>Цельнозерновые макароны с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
											<li>Творожный Чизкейк с апельсиновой цедрой на печеной тыкве</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1300 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Отварная телятина, дайкон и лаваш «СrossFood»</li>
											<li>Цельнозерновые макароны с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1100 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-sl-su.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-mo" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Оладьи из тыквы на цельнозерновой муке с ягодами Годжи  и суперпышный омлет «Облако»</li>
											<li>Рыба в панировке из льна на пару со свежим огурцом и  Диким Рисом</li>
											<li>Греча с грибами и куриной котлетой фаршированной адыгейским сыром</li>
											<li>Печеная свекла с салатом айсберг и запеченным с Киви филе домашней курицы</li>
											<li>Творожная запеканка с маком и миндальным орехом</li>
										</ol>


										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Рыба в панировке из льна на пару со свежим огурцом и  Диким Рисом</li>
											<li>Греча с грибами и куриной котлетой фаршированной адыгейским сыром</li>
											<li>Печеная свекла с салатом айсберг и запеченным с Киви филе домашней курицы</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-pow-mo.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-tu" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Овсяноблин со сливочным соусом с индейкой грибами и корнем сельдерея</li>
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай  «Массала»  на натуральном сахарозаменителе</li>
											<li>Пшеничная каша с пряными травами и рулетики из судака с сыром и зеленью</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
											<li>Творожная Бабка с Курагой и мятой </li>
										</ol>


										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки - 110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай  «Массала»  на натуральном сахарозаменителе</li>
											<li>Пшеничная каша с пряными травами и рулетики из судака с сыром и зеленью</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-we" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Рис на молоке с  ароматом Ванили печеными фруктами и жаренным арахисом</li>
											<li>Кус-кус по Мароккански с паровыми  фрикадельками из индейки</li>
											<li>Греча с творогом и куриное филе гриль с рюмкой крепкого бульона с зеленью</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты ... + Чай Fitness c имбирем и лимоном </li>
											<li>Нежирный творог с овощной пряной сальсой</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Кус-кус по Мароккански с паровыми  фрикадельками из индейки</li>
											<li>Греча с творогом и куриное филе гриль с рюмкой крепкого бульона с зеленью</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты ... + Чай Fitness c имбирем и лимоном </li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-pow-we.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-th" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Сырники без муки на Псиллиуме , подаются с   ванильно-клубничным соусом на йогурте</li>
											<li>Целые отборные фрукты</li>
											<li>Салат Цезарь на легком Фитнесс соусе с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс  с морепродуктами и томатами Черри</li>
											<li>Творожная запеканка с овощами и корениями</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Целые отборные фрукты</li>
											<li>Салат Цезарь на легком Фитнесс соусе с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс с морепродуктами и томатами Черри</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-fr" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Тосты в банане и яйце с ягодами и сметанным соусом с медом и семенами Чиа</li>
											<li>Протеиновая конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Нешлифованный Рис с телятиной в томатном соусе</li>
											<li>Кубинский салат с ананасом, филе, яблоком и йогуртом</li>
											<li>Творожный маффин с черникой и семенами льна</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Протеиновая конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Нешлифованный Рис с телятиной в томатном соусе</li>
											<li>Кубинский салат с ананасом, филе, яблоком и йогуртом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-pow-fr.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-sa" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Омлет с овощами и индейкой Су-Вид и сыром</li>
											<li>Апельсиновый фреш</li>
											<li>Отварное куриное филе с гречей, овощами и морковкой Су-Вид</li>
											<li>Салат из шпината, огурца на паровом филе морского окуня </li>
											<li>Творожная запеканка с печеными овощами и травами </li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Апельсиновый фреш</li>
											<li>Отварное куриное филе с гречей, овощами и морковкой Су-Вид</li>
											<li>Салат из шпината, огурца на паровом филе морского окуня </li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-pow-sa.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="fit-pow-su" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Блины с грушей и миндальным орехом + нежирная сметана с семенами льна</li>
											<li>Отварная телятина, дайкон и лаваш «СrossFood»</li>
											<li>Цельнозерновые макароны с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
											<li>Творожный Чизкейк с апельсиновой цедрой на печеной тыкве</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1800 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Отварная телятина, дайкон и лаваш «СrossFood»</li>
											<li>Цельнозерновые макароны с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1500 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/fit-pow-su.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>


						<div id="prem-reg-mo" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Оладьи из тыквы на цельнозерновой муке с ягодами Годжи и суперпышный омлет «Облако»</li>
											<li>Рыба в панировке из льна в пряном томатном соусе с Диким Рисом</li>
											<li>Ароматная Грибная юшка из трех видов грибов с обезжиренной сметаной и Льняным бездрожжевым хлебом</li>
											<li>Печеная свекла с выдержанной 1 год домашней брынзой и солодовыми ароматными сухарями</li>
											<li>Стейк из индейки с салатным миксом и томатным соусом</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Рыба в панировке из льна в пряном томатном соусе с Диким Рисом</li>
											<li>Ароматная Грибная юшка из трех видов грибов с обезжиренной сметаной и Льняным бездрожжевым хлебом</li>
											<li>Печеная свекла с выдержанной 1 год домашней брынзой и солодовыми ароматными сухарями</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-reg-mo.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-tu" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Овсяноблин со сливочным соусом с индейкой грибами и корнем сельдерея</li>
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай «Массала» </li>
											<li>Уха из сома, судака и бычка с саламуром и мультизлаковыми бездрожжевыми тостами</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
											<li>Котлеты из телятины с грибами на пюре из сельдерея и гречки</li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай «Массала» </li>
											<li>Уха из сома, судака и бычка с саламуром и мультизлаковыми бездрожжевыми тостами</li>
											<li>Тайский салат с Ростбифом и овощами под устричным соусом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-we" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Рис на молоке с ароматом Ванили печеными фруктами и жаренным арахисом</li>
											<li>Кус-кус по-Мароккански с фрикадельками из индейки в овощном соусе</li>
											<li>Бульон домашней курицы с рисовой лапшой + FreshRoll (лаваш, свежие овощи и зелень с сыром )</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты... + Чай Fitness c имбирем и лимоном</li>
											<li>Скумбрия печеная на шпажке с травами, подается с салатным миксом и легким сливочно-шпинатным соусом</li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Кус-кус по-Мароккански с фрикадельками из индейки в овощном соусе</li>
											<li>Бульон домашней курицы с рисовой лапшой + FreshRoll (лаваш, свежие овощи и зелень с сыром )</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты... + Чай Fitness c имбирем и лимоном</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-reg-we.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-th" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Сырники без муки на Псиллиуме, подаются с ванильно клубничным соусом на йогурте</li>
											<li>Фруктовая нарезка в соке лайма с семенами льна и миндальными хлопьями</li>
											<li>Овощной крем –суп с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс с морепродуктами и томатами Черри в легком сливочном соусе</li>
											<li>Окорок Телятины запеченный в пряных травах с овощами и корениями</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Фруктовая нарезка в соке лайма с семенами льна и миндальными хлопьями</li>
											<li>Овощной крем –суп с индейкой Су-Вид и мультизлаковыми сухариками</li>
											<li>Салатный микс с морепродуктами и томатами Черри в легком сливочном соусе</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-fr" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Тосты в банане и яйце с ягодами и сметанным соусом с медом и семенами Чиа</li>
											<li>Шоколадная конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Венгерский томатный суп Гуляш с телятиной</li>
											<li>Кубинский салат с ананасом, филе индейки, яблоком и йогуртом</li>
											<li>Печеные Наггетсы из индейки в отрубной панировке с сырным соусом и салатным миксом</li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Шоколадная конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Венгерский томатный суп Гуляш с телятиной</li>
											<li>Кубинский салат с ананасом, филе индейки, яблоком и йогуртом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-sa" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Омлет с овощами и индейкой Су-Вид + солодовые тосты с сыром</li>
											<li>Фруктовый салат в апельсиновом фреше + Живые хлебцы из пророщенной пшеницы</li>
											<li>Бульон из кролика с овощами и морковкой Су-Вид + Мультизлаковые чипсы</li>
											<li>Ореховый микс с имбирными цукатами (1200\1500 ) с финиками ( 2000\2500)</li>
											<li>Морской окунь на пару с печеными овощами и сливочным соусом Песто </li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Фруктовый салат в апельсиновом фреше + Живые хлебцы из пророщенной пшеницы</li>
											<li>Бульон из кролика с овощами и морковкой Су-Вид + Мультизлаковые чипсы</li>
											<li>Ореховый микс с имбирными цукатами (1200\1500 ) с финиками ( 2000\2500)</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-reg-su" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Блины с грушей и миндальным орехом + нежирная сметана с семенами льна</li>
											<li>Начос с телятиной и лавашем «СrossFood»</li>
											<li>Суп-крем из шпината  и соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
											<li>Запеченная утка с тыквой, апельсином и имбирем</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Начос с телятиной и лавашем «СrossFood»</li>
											<li>Суп-крем из шпината  и соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с индейкой Су-Вид, маринованными лисичками, маслятами и твердым сыром</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-reg-su.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-mo" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Оладьи из тыквы на цельнозерновой муке с ягодами Годжи и суперпышный омлет «Облако»</li>
											<li>Фасолевый микс в пряном томатном соусе с Диким Рисом</li>
											<li>Ароматная Грибная юшка из трех видов грибов с обезжиренной сметаной и Льняным бездрожжевым хлебом</li>
											<li>Печеная свекла с выдержанной 1 год домашней брынзой и солодовыми ароматными сухарями</li>
											<li>Стейк из печеного тофу Гриль с салатным миксом и томатным соусом</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Фасолевый микс в пряном томатном соусе с Диким Рисом</li>
											<li>Ароматная Грибная юшка из трех видов грибов с обезжиренной сметаной и Льняным бездрожжевым хлебом</li>
											<li>Печеная свекла с выдержанной 1 год домашней брынзой и солодовыми ароматными сухарями</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-veg-mo.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-tu" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Овсяноблин со сливочным соусом с соевым мясом, грибами и корнем сельдерея</li>
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай «Массала»</li>
											<li>Яркий Овощной суп с рисом и зеленью</li>
											<li>Тайский салат с Грибами Гриль и овощами под устричным соусом</li>
											<li>Котлета овощная с сыром на пюре из сельдерея и гречки</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Хлеб творожно-отрубной со спирулиной и Индийский Чай «Массала»</li>
											<li>Яркий Овощной суп с рисом и зеленью</li>
											<li>Тайский салат с Грибами Гриль и овощами под устричным соусом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-we" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Рис на молоке с ароматом Ванили печеными фруктами и жаренным арахисом</li>
											<li>Кус-кус по Мароккански с черной фасолью в овощном соусе</li>
											<li>Овощной Мисо Бульон со спаржевой фасолью и рисовой лапшой + FreshRoll ( лаваш, свежие овощи и зелень с сыром )</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты... + Чай Fitness c имбирем и лимоном</li>
											<li>Шампиньоны Гриль  на шпажке с травами, подается с салатным миксом и легким сливочно-шпинатным соусом</li>

										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Кус-кус по Мароккански с черной фасолью в овощном соусе</li>
											<li>Овощной Мисо Бульон со спаржевой фасолью и рисовой лапшой + FreshRoll ( лаваш, свежие овощи и зелень с сыром )</li>
											<li>Заготовки СrossFood ( высушенные при 40*С ) фрукты, хлебцы, пастила, натуральные конфеты... + Чай Fitness c имбирем и лимоном</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-veg-we.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-th" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Сырники без муки на Псиллиуме, подаются с ванильно клубничным соусом на йогурте</li>
											<li>Фруктовая нарезка в соке лайма с семенами льна и миндальными хлопьями</li>
											<li>Овощной	 крем –суп с соевым мясом Гриль и мультизлаковыми сухариками</li>
											<li>Салатный микс с белыми грибами и томатами Черри в легком сливочном соусе</li>
											<li>Запеканка творожная с овощами и корениями</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Фруктовая нарезка в соке лайма с семенами льна и миндальными хлопьями</li>
											<li>Овощной	 крем –суп с соевым мясом Гриль и мультизлаковыми сухариками</li>
											<li>Салатный микс с белыми грибами и томатами Черри в легком сливочном соусе</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-fr" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Тосты в банане и яйце с ягодами и сметанным соусом с медом и семенами Чиа </li>
											<li>Шоколадная конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Венгерский томатный суп Гуляш с белыми грибами</li>
											<li>Кубинский салат с ананасом, яйцом, яблоком и йогуртом</li>
											<li>Печеные Наггетсы из соевого мяса в отрубной панировке с сырным соусом и салатным миксом</li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Шоколадная конфета «СrossFood» и облепиховый чай с мятой</li>
											<li>Венгерский томатный суп Гуляш с белыми грибами</li>
											<li>Кубинский салат с ананасом, яйцом, яблоком и йогуртом</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/prem-veg-fr.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-sa" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Омлет с овощами и грибами + солодовые тосты с сыром</li>
											<li>Фруктовый салат в апельсиновом фреше + Живые хлебцы из пророщенной пшеницы</li>
											<li>Чичевичный суп с овощами и морковкой Су-Вид + Мультизлаковые чипсы</li>
											<li>Ореховый микс с имбирными цукатами ( 1200\1500 ) с финиками ( 2000\2500 )</li>
											<li>Творожная запеканка с печеными овощами и соусом Песто </li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Фруктовый салат в апельсиновом фреше + Живые хлебцы из пророщенной пшеницы</li>
											<li>Чичевичный суп с овощами и морковкой Су-Вид + Мультизлаковые чипсы</li>
											<li>Ореховый микс с имбирными цукатами ( 1200\1500 ) с финиками ( 2000\2500 )</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>

						<div id="prem-veg-su" class="tab-block__tab-item disable">
							<div class="cuisine__item">
								<div class="cuisine__item-text-cont">
									<div data-cuisine-qount="5" class="">
										<ol class="cuisine__item-text-menu">
											<li>Блины с грушей и миндальным орехом + нежирная сметана с семенами льна</li>
											<li>Начос с черной фасолью и лавашем «СrossFood»</li>
											<li>Суп-крем из шпината с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с соевым мясом, маринованными лисичками, маслятами и твердым сыром</li>
											<li>Чечевичная каша с томатным Карри </li>
										</ol>

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">2000 ккал</u></li>
											<li>Белки -110 г</li>
											<li>Жиры - 73 г</li>
											<li>Углеводы - 160г</li>
										</ul>									
									</div>								

									<div data-cuisine-qount="3" class="disable">
										<ol class="cuisine__item-text-menu">
											<li>Начос с черной фасолью и лавашем «СrossFood»</li>
											<li>Суп-крем из шпината с соте из бейби осьминожек и зерен черной горчицы</li>
											<li>Пьемонтский салат с соевым мясом, маринованными лисичками, маслятами и твердым сыром</li>
										</ol>								

										<ul class="cuisine__item-text-calories">
											<li><u class="cuisine__item-text-highlited">1600 ккал</u></li>
											<li>Белки - 70 г</li>
											<li>Жиры - 43 г</li>
											<li>Углеводы - 130г</li>
										</ul>									
									</div>
								</div>

								<div class="cuisine__item-img-cont">

									<div class="cuisine__item-img-block">
										<img class="cuisine__item-img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/cuisine/custom.jpg" alt="Салат">
									</div>
									<div class="cuisine__item-price-block">
										<span data-cuisine-qount="3" class="cuisine__item-price-text disable">290 грн</span>
										<span data-cuisine-qount="5" class="cuisine__item-price-text">350 грн</span>
									</div>
									
								</div>

							</div>
						</div>
	
				</div>

			</div><!-- 

			 --><div class="cuisine-notation_position">
				<p class="cuisine-notation">Пробный день - 250 грн</p>
			</div>
			
		</div>
	</section>

<!-- 	<section class="our-video">
		<div class="wrapper">
			
		</div>
	</section>


	<section id="discount" class="discount">
		<div class="wrapper">
			
		</div>
	</section> -->


	<section class="reviews reviews_background">
		<div class="wrapper reviews_position">
			<div id="reviews" class="title-3">
				<h3 class="title-3__h3 title-3__h3_dark hr-line">Отзывы</h3>	
			</div>
			
			<div id="reviews-slider" data-slides-onpage="3" class="slider reviews-slider">
				<div id="reviews-slider-inner" class="slider__inner">
					<div data-slide class="slider__item reviews-slider__item">
						<img data-meta-node data-action="slide-img" class="reviews-slider__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/reviews/slide-1-1.jpg" alt="Отзыв покупателя">
					</div><!-- 
					 --><div data-slide class="slider__item reviews-slider__item">
						<img data-meta-node data-action="slide-img" class="reviews-slider__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/reviews/slide-1-2.jpg" alt="Отзыв покупателя">
					</div><!-- 
					 --><div data-slide class="slider__item reviews-slider__item">
						<img data-meta-node data-action="slide-img" class="reviews-slider__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/reviews/slide-1-3.jpg" alt="Отзыв покупателя">
					</div><!-- 
					 --><div data-slide class="slider__item reviews-slider__item">
						<img data-meta-node data-action="slide-img" class="reviews-slider__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/reviews/slide-1-4.jpg" alt="Отзыв покупателя">
					</div><!-- 
					 --><div data-slide class="slider__item reviews-slider__item">
						<img data-meta-node data-action="slide-img" class="reviews-slider__img" src="<?php bloginfo("template_url"); ?>/src/img/cnt/reviews/slide-1-5.jpg" alt="Отзыв покупателя">
					</div>
				</div>
				
				<div id="reviews-slider-nav-arw">
					<div data-action="left" data-meta-node class="slider__arrow  slider_arrow-left reviews-slider__arrow reviews-slider__arrow-left"></div>
					<div data-action="right" data-meta-node class="slider__arrow  slider_arrow-right reviews-slider__arrow reviews-slider__arrow-right"></div>
				</div>
					
				<div id="reviews-slider-nav-btn" class="slider__btn-nav reviews-slider__btn-nav"></div>
			</div>



			<div class="reviews-hashtag-block">
				<p class="reviews-hashtag-block__text">больше отзывов по хэштегам 
					<a class="reviews-hashtag-block_link reviews-hashtag-block_link-vk" href="https://vk.com/search?c%5Bq%5D=%23crossfood_od&c%5Bsection%5D=statuses" target="_blank">#crossfood_od</a>
					<a class="reviews-hashtag-block_link reviews-hashtag-block_link-insta" href="https://www.instagram.com/explore/tags/crossfood_od/" target="_blank">#crossfood_od</a>
					<a class="reviews-hashtag-block_link reviews-hashtag-block_link-fb" href="https://www.facebook.com/hashtag/crossfood_od?ref=ts&fref=ts" target="_blank">#crossfood_od</a>
				</p>
			</div>
			
			
		</div>
	</section>

	<?php get_footer() ?>